...
##    source: str,
    ...
    redownload: bool = False,                                   ) -> list[xr.Dataset]:
    ...
    for file in file_list:
        ...
        if file.lower().endswith(".nc"):
            ...
            # Time handling
            try:
                df = df.apply(lambda col: col.astype(str).str.replace(',', '', regex=False).astype(float))
                # df['Decimal year'] = df['Decimal year'].astype(str).str.replace(',', '',regex=False).astype(float)
                df['TIME'] = df['Decimal year'].apply(lambda x: datetime.datetime(int(x), 1, 1) + datetime.timedelta(days=(x - int(x)) * (datetime.datetime(int(x)+1, 1, 1) - datetime.datetime(int(x),1, 1)).days))
